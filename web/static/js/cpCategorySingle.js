...
pageModel = {
	...
	"buildAttachments" : function(){
		var html = [
			'<div class="addButton">',
			'<div class="img"><span>Добавить изображение</span></div>',
			'</div>'
		];
		...
		for(var c=0; c<this.attachments.length; c++){
			...
			var tmp2 = [
				...
				'<div uid="'+tmp.uid+'"  class="button delete" title="Удалить"><span>D</span></div>',
				'<a uid="'+tmp.uid+'" class="button full"  href="'+location.rooturl + tmp.path+'" title="Полный размер" target="_blank"><span>F</span></a>',
				'</div>',
				'</div>'
			];
			html.push(tmp2.join(''));
...
$(document).ready(function(){
	...
	$(".thumbnailsRow").on("click", ".addButton", function(){
	...
	$(".thumbnailsRow").on("click", ".thumbnail .button.delete", function(){
